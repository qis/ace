...
* `clang-format-linux.tar.gz` - latest version of clang-format
...
* `clang-format-windows.tar.gz` - latest version of clang-format
...
# Install ports.
# make -C src/ports PORTS="tbb zlib ..." install
make -C src/ports install

# Check ports linkage.
# make -C src/ports PORTS="tbb zlib ..." check
...
docker cp ace:/opt/ace/clang-format-linux.tar.gz clang-format-linux.tar.gz
...
copy "\\wsl$\Debian\opt\ace\clang-format-linux.tar.gz" clang-format-linux.tar.gz
...
rem Install ports.
rem make -C src/ports PORTS="tbb zlib ..." install
make -C src/ports install

rem Check ports linkage.
rem make -C src/ports PORTS="tbb zlib ..." check
make -C src/ports check
