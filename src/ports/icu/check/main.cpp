...
#ifndef ICU_USE_BUILD_DIRECTORY
#define ICU_USE_BUILD_DIRECTORY 0
#endif

int main(int argc, char* argv[]) {
  icu::ErrorCode ec;

  if (ICU_USE_BUILD_DIRECTORY) {
  ...
  auto size = u_charName(code, U_UNICODE_CHAR_NAME, name.data(),
    static_cast<int32_t>(name.size()), ec);
  if (size > static_cast<int32_t>(name.size())) {
    ec.reset();
    name.resize(static_cast<std::size_t>(size) + 1);
    size = u_charName(code, U_UNICODE_CHAR_NAME, name.data(),
      static_cast<int32_t>(name.size()), ec);
  }
  if(ec.isFailure()) {
  ...
  u_getVersion(va);
  std::cout << "icu: "
    << static_cast<unsigned>(va[0]) << '.'
    << static_cast<unsigned>(va[1]) << std::endl;
