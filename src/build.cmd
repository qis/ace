...
if not exist tools-windows.tar.gz make tools
if not exist tools-windows.tar.gz exit /b 1

rem Create clang-format archive.
if not exist tools-windows-clang-format.tar.gz make clang-format
if not exist tools-windows-clang-format.tar.gz exit /b 1

rem Create sysroot archive.
if not exist sys-x86_64-pc-windows-msvc.tar.gz make sys
