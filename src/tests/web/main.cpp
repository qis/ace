...
__attribute__((export_name("set_errno")))
...
// __isatty: (fd) => {
//   exports.set_errno((fd !== files.stdout && fd !== files.stderr) ? errno.EBADF : errno.ENOTTY);
//   return 0;
// },
//
...
// lseek(2) repositions the file descriptor 'fd' offset to 'offset' according to the directive 'whence'.
//
// - On success, returns the resulting offset location in bytes from the beginning of the file.
// - On failure, returns '-1' and sets 'errno'.
//
// __lseek: (fd, offset, whence) => {
//   exports.set_errno((fd !== files.stdout && fd !== files.stderr) ? errno.EBADF : errno.ESPIPE);
//   return -1;
// },
//
...
// writev(2) writes 'iovcnt' buffers described by 'iov' to the file descriptor 'fd'.
//
// - On success, returns the resulting offset location in bytes from the beginning of the file.
// - On failure, returns '-1' and sets 'errno'.
//
...
//   let text = "";
//   const iov_size = 2;
//   const iov_data = new Uint32Array(this.exports.memory.buffer, iov, iovcnt * iov_size);
//   for (var i = 0; i < iovcnt; i++) {
//     const iov_base = iov_data[i * iov_size + 0];
//     const iov_size = iov_data[i * iov_size + 1];
//     if (iov_size > 0) {
//       const iov_data = new Uint8Array(this.exports.memory.buffer, iov_base, iov_size);
//       try {
