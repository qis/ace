...
if [ ! -d sys/linux ]; then
  ...
  rm -rf \
    ...
    sys/linux/usr/include/x86_64-linux-gnu/ffi.h \
    sys/linux/usr/include/x86_64-linux-gnu/ffitarget.h \
    sys/linux/usr/lib/x86_64-linux-gnu/libffi.a \
    sys/linux/usr/lib/x86_64-linux-gnu/libffi.so \
    sys/linux/usr/lib/x86_64-linux-gnu/libffi_pic.a \
    sys/linux/usr/lib/x86_64-linux-gnu/pkgconfig/libffi.pc \
    sys/linux/usr/lib/x86_64-linux-gnu/libffi.so.7 \
    sys/linux/usr/lib/x86_64-linux-gnu/libffi.so.7.1.0

  sed -iE 's/^Requires.private: libffi$//' sys/linux/usr/lib/x86_64-linux-gnu/pkgconfig/wayland-client.pc
...
if [ ! -f share/license.rtf ]; then
  cp -a src/license.rtf share/
fi

if [ ! -f share/license.txt ]; then
  cp -a src/license.txt share/
fi
...
if [ ! -f Ace/mingw.cmake ]; then
  rm -rf Ace/res
  mkdir -p Ace/res/triplets Ace/share
  ...
  cp -a src/license.rtf src/license.txt Ace/share/
